...
class F1TenthController(Node):
        ...
        if tube:
            ...
            if distance_error < self.distance_tolerance:
                ...
                status_message = (
                    ...
                    f'Current Pose: ({self.current_x:.2f}, {self.current_y:.2f}) | Target Pose: ({target_x:.2f}, {target_y:.2f})'
                )
    ...
    def plot_thread_run(self):
        ...
        """
        while rclpy.ok() and not self.timer.is_canceled():
            if self.t_history:
                try:
                    # Update X vs Y Plot
...
def main(args=None):
    ...
    controller_node = F1TenthController(2)
